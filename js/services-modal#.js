...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function resetMedia() {
    ...
    youtubeRenderToken++;
    if (youtubeLoadTimer) { clearTimeout(youtubeLoadTimer); youtubeLoadTimer = null; }
  }
  ...
  function renderMediaItem(media, data) {
    ...
    if (media && media.type === 'youtube' && media.id) {
      ...
      // Some ad blockers / antivirus "web shield" tools silently block the
      // /embed/ request without YouTube ever getting the chance to report an
      // error back to us. If we don't hear an "onReady" message within a few
      // seconds, assume it's stuck/blocked and fall back automatically.
      var thisToken = youtubeRenderToken;
      youtubeLoadTimer = setTimeout(function () {
        if (thisToken === youtubeRenderToken && currentServiceData) {
          showFallback(currentServiceData, media.id);
        }
      }, 4000);
      return;
    }
